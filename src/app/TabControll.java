...
public class TabControll {
	...
	public TabControll() {
		...
		try {
			...
			System.out.println("Wczytano bazê klientów oraz us³ug");
		} catch (Exception e) {
			tabCustomers = new TabCustomers();
			services = new TabService();
			System.out.println("Utworzono bazê klientów bazê klientów oraz us³ug");
			e.printStackTrace();
		}
	}
	...
	private enum Option {
		EXIT(0, "Wyjœcie z programu"), ADD_INDIVIDUAL(1, "Dodanie klienta indywidualnego"), PRINT_INDIVIDUAL(2,
				"Wyœwietlenie klientów indywidualnych"), ADD_SOHO(3, "Dodanie klienta biznesowego"), PRINT_SOHO(4,
						"Wyœwielenie klitnów biznesowych"), PHONE(5,
